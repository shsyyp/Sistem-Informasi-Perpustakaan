    ...
    <div class="card">
        <div class="d-flex flex-column flex-md-row p-4">
            <div class="d-flex flex-grow-1 head-label text-center">
                <h5 class="card-title mb-0">Edit Eksemplar Buku</h5>
            </div>
        ...
        <div class="card-body pt-0">
            <form action="{{ route('eksemplar.update', $data->id) }}" class="form" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select name="buku_id" class="form-control @error('buku_id') is-invalid @enderror">
                                @foreach ($bukuList as $buku)
                                    <option value="{{ $buku->id }}" {{ $data->buku_id == $buku->id ? 'selected' : '' }}>{{ $buku->judul }}</option>
                                @endforeach
                            </select>
